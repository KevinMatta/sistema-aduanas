...
export class PantallasService implements DataService {
    ...
    getPantallas(): Observable<Pantalla[]> {
        return this.http.get<APIResponse<Pantalla[]>>(this.Url).pipe(
    ...
    getEsquemas(): Observable<Esquema[]> {
        return this.http.get<APIResponse<Esquema[]>>(this.Url + "Esquemas").pipe(
            map(response => this.mapEsquemasResponse(response.data))
        );
    }
